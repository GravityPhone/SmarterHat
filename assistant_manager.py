...
class AssistantManager:
    ...
    def create_thread(self):
        try:
            ...
            logger.error(f"Failed to create a thread: {e}")
            return None
    ...
    def run_assistant(self, thread_id, assistant_id, instructions):
        try:
            ...
            logger.info(f'Run status immediately after running assistant: {run_status}')
    ...
    def handle_pending_state(self, run_id):
        ...
        logger.info('Handled pending state.')
    ...
    def handle_queued_state(self, run_id):
        ...
        while time.time() - start_time < timeout:
            try:
                ...
                logger.info(f"Run status at {time.time()}: {run_status.status}")
                # Handle 'queued' state
    ...
    def retrieve_most_recent_message(self, thread_id):
        try:
            ...
            logger.error(f"Failed to retrieve the most recent message from thread {thread_id}: {e}")
            return None
    ...
    def check_run_status(self, thread_id, run_id):
        try:
            ...
            status = run.status
            logger.info(f'Checking run status: {status}')
            return status
        except Exception as e:
            logger.error(f"Failed to check run status for run {run_id} in thread {thread_id}: {e}")
            return None
    ...
    def interact_with_assistant(self, transcription):
        ...
        logger.info(f"Assistant run initiated with ID: {run_id}")
        ...
        if run_status == 'pending':
            ...
            logger.warning('Run is in an unknown state.')
        if self.check_run_status(last_thread_id, run_id):
            ...
            logger.info(f"Played back the assistant's response: {processed_response}")
        else:
            logger.error("Assistant processing failed or timed out.")
            return None
